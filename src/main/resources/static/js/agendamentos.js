...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const API = {
    listar:   () => fetchJSON("/agendamentos/api"),
    obter:    (id) => fetchJSON(`/agendamentos/api/${id}`),

    // cria via JSON; se 415, cai no /salvar (form-urlencoded)
    criar:    async (p) => {
      try {
        return await fetchJSON(`/agendamentos/api`, { method: "POST", body: JSON.stringify(p) });
      } catch (e) {
    ...
    atualizar: async (id, p) => {
      try {
        return await fetchJSON(`/agendamentos/api/${id}`, { method: "PUT", body: JSON.stringify(p) });
      } catch (e) {
        if (e.status === 415 || e.status === 404 || e.status === 405) {
    ...
    inativar: async (id) => {
      try {
        return await fetchJSON(`/agendamentos/api/${id}`, { method: "DELETE" });
      } catch (e) {
    ...
    profDisp: (inicioISOWithOffset, duracaoMin, servicoId) => {
      const url = new URL(location.origin + `/agendamentos/api/profissionais-disponiveis`);
